...
const MeusAgendamentosItem : React.FC<BookingItemBarbershopProps> = ({
    MeusAgendamentos,
    onRemoveBooking,
  }) => {
    console.log("servicoProfissonal", MeusAgendamentos);
  
    const { user } = useAuth();
    const barbeariaId = user?.barbearia?.id || null;
  
    const bookingDate = new Date(
      `${MeusAgendamentos.data}T${MeusAgendamentos.hora}`
    );
    const isBookingConfirmed = MeusAgendamentos.status === "Confirmado";
    const [isDeleteLoading, setIsDeleteLoading] = useState(false);
    const [telefones, setTelefones] = useState<Telefone[]>([]);
    const [barbearia, setBarbearia] = useState<Barbearia | null>(null);
    const { servico, profissional } = MeusAgendamentos.profissionalServico;
    ...
    useEffect(() => {
      const fetchTelefones = async () => {
        try {
          const response = await api.get(
            `/telefone/cliente/${MeusAgendamentos.cliente.id}`
          );
          setTelefones(response.data);
          console.log("telefonecliente", MeusAgendamentos.cliente.id);
        } catch (error) {
      ...
      fetchTelefones();
      fetchBarbearia();
    }, [MeusAgendamentos.cliente.id, barbeariaId]);
              ...
              <CardContent className="p-0 flex py-0 debug-border">
                <div className="flex flex-col gap-2 py-5 flex-[3] pl-5 debug-border">
                  ...
                  <div className="flex items-center gap-2">
                    <Avatar>
                      <AvatarImage
                        src={MeusAgendamentos.cliente.foto}
                        alt={MeusAgendamentos.cliente.nome.charAt(0) || ""}
                        width={40}
                        className="rounded-full"
                      />
                      <AvatarFallback>
                        {MeusAgendamentos.cliente.nome.charAt(0)}
                      </AvatarFallback>
                    </Avatar>
    
                    <h3 className="capitalize">
                      {MeusAgendamentos.cliente.nome}{" "}
                      {MeusAgendamentos.cliente.sobreNome}
                    </h3>
                  </div>
                </div>
                  ...
                  <AlertDialogContent className="w-[90%]">
                    ...
                    <AlertDialogFooter className="flex-row gap-3">
                      ...
                      <AlertDialogAction
                        ...
                        className="w-full"
                        onClick={() => removeAgendamento(MeusAgendamentos.id)}
                      >
                        {isDeleteLoading && (
...
export default MeusAgendamentosItem